...
#Second end point is to select skin type and receive skincare routine using
# API POST method
@app.route('/skincare', methods=['POST'])
def get_skincare_routine():
    data = request.get_json() # This part extracts the JSON data from the
# incoming POST request(user input)
    skin_type = data.get("skin_type")  # This part always makes sure there
# is an input from the user
    if skin_type in skincare_data:
        routine = skincare_data.get(skin_type) # This looks up the skin
# care routine from the skincare dictionary associated with the skin type
        return jsonify({"skin_type": skin_type, "skincare_routine":
routine}) # This gives the user a JSON response which shows them the skin
# routine for their selected skin type.
    else:
        return jsonify({"error": "Invalid Skin type. Please Choose from the available options"}), 400
# Even through users will only have option of 6 skin types; basic error
# handling is good to have in an API
# Third Endpoint is to list all available skin types using API GET method
# We need to check if there are any skin types available and dict isnt
# empty then we produce a message that states whats available.
...
# Here we use the keyword Global, so we can access skincare_data outside of
# this function and able to delete data.
# the {} indicates the dictionary has been emptied out
# the result will be the Key Message with Value All Skincare data has been
# deleted.
